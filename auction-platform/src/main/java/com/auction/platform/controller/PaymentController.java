...
@Tag(name = "Payments", description = "Payment initiation, webhook confirmation, refunds, history")
public class PaymentController {
    ...
    public ResponseEntity<ApiResponse<PaymentResponse>> initiate(
        ...
        return ResponseEntity.ok(ApiResponse.success("Payment initiated",
                paymentService.initiatePayment(principal.getUser(), auctionId)));
    }

    @PostMapping("/webhook")
    @Operation(summary = "Gateway webhook (server-to-server, signature-verified — not for direct client use)")
    public ResponseEntity<Void> webhook(
            HttpServletRequest request,
            @RequestHeader(value = "X-Gateway-Signature", required = false) String signature) throws IOException {
        // Raw bytes captured here, BEFORE any JSON parsing, so signature verification is
        // computed against exactly what the gateway signed — see Phase 11 design notes.
    ...
    @Operation(summary = "Refund a successful payment (seller or admin only)")
    public ResponseEntity<ApiResponse<PaymentResponse>> refund(
            @AuthenticationPrincipal CustomUserDetails principal, @PathVariable Long paymentId) {
        return ResponseEntity.ok(ApiResponse.success("Payment refunded",
                paymentService.refund(principal.getUser(), paymentId)));
    }

    @GetMapping("/my-history")
    @Operation(summary = "Your payment history, paginated")
    public ResponseEntity<ApiResponse<Page<PaymentResponse>>> myHistory(
        ...
        return ResponseEntity.ok(ApiResponse.success("Payment history fetched",
                paymentService.getMyPaymentHistory(principal.getUser(), pageable)));
    }

    @GetMapping("/{paymentId}")
    @Operation(summary = "Get a single payment (buyer, seller, or admin only)")
    public ResponseEntity<ApiResponse<PaymentResponse>> getById(
            @AuthenticationPrincipal CustomUserDetails principal, @PathVariable Long paymentId) {
        return ResponseEntity.ok(ApiResponse.success("Payment fetched",
                paymentService.getById(principal.getUser(), paymentId)));
    }
}
